...
func (r *RaftImpl) Configure(servId int32, configPath string, currTimeout int, leaderchan chan int32, prob float64, delay int, exit <-chan int) {
	...
	r.timer = time.NewTimer(time.Duration(r.timeout) * time.Millisecond)
	r.commitIndex = -1
	r.lastApplied = -1
		...
		case msg := <-r.server.Inbox():
			...
			if msg.MsgType == COMMAND {
				if r.leaderPid == -1 {
					...
					cmdMsg.Cmd = msg.Cmd
					r.server.Outbox() <- cmdMsg
				}
			} else if msg.MsgType == REQUESTVOTE && msg.TermId > r.term {
				//Send OK
				if msg.Msg.PrevLogTerm > r.log[r.LastIndex].Term || (msg.Msg.PrevLogTerm == r.log[r.LastIndex].Term && msg.Msg.PrevLogIndex >= r.LastIndex) {
...
func (r *RaftImpl) candidate() {
	...
	appendEntry.PrevLogIndex = r.LastIndex
	appendEntry.PrevLogTerm = r.log[r.LastIndex].Term
	msg.Msg = appendEntry
...
func (r *RaftImpl) leader() {
	...
	r.timer.Reset(time.Duration(50) * time.Millisecond)
	//r.isLeader = true
	r.CurrLeader() <- r.Pid()
	for {
		select {
		case msg := <-r.server.Inbox():
			switch {
			case msg.MsgType == COMMAND:
				r.TRACE.Println("New command received at leader")
				r.Inbox() <- msg.Cmd
			case msg.MsgType == REQUESTVOTE:
				if msg.TermId > r.term {
					r.changeTerm(msg.TermId)
		...
		case cmd := <-r.Inbox():
			...
			logItem.Term = r.Term()
			logItem.Data = cmd
			r.AddToLog(logItem, (r.LastIndex + 1))
			r.LastIndex = r.LastIndex + 1
